...
class Beacon_sim():
    ...
    def __init__(self, dt=0.2, pos=[0.0,0.0,0.0], vel=[0.0,0.0,0.0],ruido=1):
        ...
        for i in range(self.Beacons_num):
            self.Beacons_dist.append(math.sqrt((self.pos[0] - self.Beacons[i]['x'])**2+(self.pos[1] - self.Beacons[i]['y'])**2+(self.pos[2] - self.Beacons[i]['z'])**2))
    ...
    def update_position(self):
        ...
        for i in range(len(self.Beacons_dist)):
            self.Beacons_dist[i] = math.sqrt((self.pos[0] - self.Beacons[i]['x'])**2+(self.pos[1] - self.Beacons[i]['y'])**2+(self.pos[2] - self.Beacons[i]['z'])**2) + self.ruido*randn()
            Beacon_dist_matriz.append(self.Beacons_dist[i])
        
        for i in range(len(self.Beacons_dist)):
            Beacon_dist_matriz.append(self.Beacons_dist_last[i])

        return np.array([Beacon_dist_matriz]).T
